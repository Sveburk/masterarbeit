...
\node[large, left=1.5cm of roles] (orgmatch) {\textbf{Organization-Matcher.py}\\(Organisationen normalisieren)};

\node[large, above=1cm of orgmatch] (rolematch) {\textbf{Role-Matcher.py}\\(Rollen normalisieren)};
...
\node[draw=none, inner sep=0.5cm, fit=(split)(orgmatch)(count)] (group) {};
...
\node[large, left=2.5cm of roles] (orgmatch) {\textbf{Organization-Matcher.py} \\ (Liefert normalisierte \\ Organisationen)};
...
\draw[arrow] (orgmatch.east) -- (roles.west);
\node[draw=none, rounded corners, inner sep=0.5cm, fit=(split)(orgmatch)(count)] (group) {};
\node[draw=gray, dashed, rounded corners, inner sep=0.5cm, fit=(split)(count)] (group) {};
\end{scope}
\end{tikzpicture}
  }
...
\begin{minipage}[t]{0.32\textwidth}
  ...
  Das ist n Test wie viel Text ich neben diese Grafik beomme. Wollen wir mal sehen, wa?
...
\begin{scope}[shift={(11.1,0.45)}, node distance=0.3cm]
...
\node[process, right=2.5cm of parse] (LoadPers) {\textbf{load\_known\_persons\_from\_csv} \\ Lädt CSV, erzeugt KNOWN\_PERSONS};
...
\draw[arrow] (LoadPers.west) -- ++(-1.9,0) |- (split.east);
...
\end{tikzpicture}
  ...
  \captionof{figure}{das is n Test}
\end{minipage}
