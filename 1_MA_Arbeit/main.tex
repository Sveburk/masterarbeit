...
\newcommand{\tagbox}[1]{\tcbox[tagstyle]{#1}}
\usepackage{array}
\usepackage{minted}
\usepackage{capt-of}
\usepackage{wrapfig}
...
Ziel ist es, dieses Archiv digital zugänglich, die beteiligten Personen sowie deren Netzwerke und deren geographische Ausdehnung sichtbar zu machen.
...
begünstigte früh die Ansiedlung von Mühlen, Hammerwerken und Schmieden entlang des Flusslaufs. Der Rhein bietet dazu mit seiner 
Drahtseil-Fähre eine bedeutende Verkehrs- und Handelsverbindung, die bis zum Ersten Weltkrieg privat betrieben wurde.
...
die zentrale Grundlage für die vorliegende Untersuchung.
...
Die vorliegende Arbeit knüpft an zwei Vorarbeiten an, die in den Jahren 2019 und 2022 am Departement Geschichte der Universität Basel durchgeführt
...
Darüber hinaus werden eigene Recherchen in den Beständen des \textit{Bundesarchivs – Militärarchiv Freiburg}\cite{hollmann_freiburg_2025} durchgeführt. Ergänzende Recherchen 
...
bis auf wenige Meter\footnote{im Fall von Militäreinheiten} oder auf Gebäudeebene genau rekonstruieren. Diese Erkenntnisse werden mit historischen Karten, Luftbildern und Ortsrecherchen verknüpft
...
und bilden eine wesentliche Grundlage für die erweiterte, automatisierte Pipeline, die im Folgenden vorgestellt wird. Dazu gehören auch die Verbandsabzeichen, 
...
Webscraping nach Informationen zu diesem Korpus ist nahezu unmöglich. Ausgeführt werden diese Probleme unter anderem bei 
...
\textit{Knoten und Kanten III}\cite{gamper_knoten_2015} verwiesen. Dieser  verdeutlicht, dass die historische Netzwerkanalyse zwar von einem interdisziplinär etablierten Methodenkanon 
profitiert, jedoch nach wie vor substanziellen Herausforderungen gegenübersteht. Dazu zählen die Fragmentierung historischer Quellen, der hohe manuelle 
Erfassungsaufwand und methodische Desiderate im Umgang mit zeitlichen und räumlichen Dimensionen. Erschwerende Faktoren einer systematischen Erfassung 
...
die während des Zweiten Weltkriegs verfasst wurden. Der Analysezeitraum erstreckt sich dementsprechend zwischen dem 01. September 1939 
...
In dem Lagerraum der \textit{New Gospel Singers Murg}, dem Nachfolgeverein des \textit{Männerchors Murg}, 
...
\textit{\enquote{Männerchor Akten 1946--1950}} den grössten Zeitraum abdeckt. Darüber hinaus bietet er das Potential, 
...
Der Ordner umfasst insgesamt 780 Seiten. Deren Inhalt kann als \enquote{Protokoll}, \enquote{Brief}, \enquote{Postkarte}, \enquote{Rechnung}, 
...
\enquote{Heftstreifen}, auch \enquote{Aktendulli} genannt, zusammengeheftet. In der Plastikversion, wie er in diesen Akten vorliegt, wurde er bereits 1938 patentiert\cite[vgl.][]{noauthor_heftstreifen_2023}.
...
\begin{minipage}[t]{0.49\textwidth}
  \setstretch{1.5}
  \justifying
  Für diese Arbeit wurde ein Korpus selektiert, dessen Auswahl in~\nameref{section:Korpus} näher beschrieben wird. Nachfolgende Zahlen beschränken sich auf alle Dokumente, die Während des zweiten Weltkriegs geschrieben wurden.
  ...
  Diese Auflistung in der Datei \textit{Akten\_Gesamtübersicht.csv} erlaubt die Zuordnung folgender Kategorien: Briefe, Postkarten, Protokolle, Regierungsdokumente, Zeitungsartikel, Rechnungen und Offerten. 
\end{minipage}%
\hfill
\begin{minipage}[t]{0.49\textwidth}
  \centering
...
\par\vspace{0.06cm}
\noindent
Die Verteilung ist ungleichmässig: Briefe bilden mit 282 von 381 Seiten die grösste Kategorie; Rechnungen und Offerten sind jeweils nur einseitig vertreten.\\
\par\vspace{0.06cm}
\noindent
...
\subsubsection{Sichtung \& Kategorisierung zu Akten}
Für diese Arbeit werden alle Seiten in dem Ordner \textit{\enquote{Männerchor Akten 1925--1944}} 
zweimal gesichtet und gelesen. Beim ersten Durchgang wird explorativ nach zusammenhängenden Unterlagen gesucht, die im Anschluss zu einer Akte gefasst werden können.
...
\begin{itemize}
  ...
  \item gleiches Datum in direkt aufeinanderfolgenden Seiten
  \item gleiches Thema in direkt aufeinanderfolgenden Seiten
\end{itemize}
Auf dieser Grundlage wird eine Aktenübersicht\footnote{genannt Akten\_Gesamtübersicht.csv; in den Projektdaten} im CSV-Format erstellt. Diese Tabelle setzt sich zusammen aus Aktennummern, die wiederum die Reihenfolge innerhalb des ursprünglichen Ordners beschreiben. In diesem ersten Schritt gilt die Lage als 
...
Vorgreifend soll auch die zweite Quellensichtung beschrieben sein, in der die Daten in der CSV um Metadaten auf Seitenebene und aus Transkribus ergänzt werden. Die Kategorisierung findet also in einem parallelen Prozess mit der Digitalisierung statt.
Hierfür werden die digitalen Akten auf Seitenebene genau aufgebaut. Dem zugrunde liegt eine interne Seiten-ID, die den Aktennamen und die Position innerhalb der Akte kombiniert (Bsp: Akt\_078\_S001.jpg). 
Ab dem Zeitpunkt des Uploads bei Transkribus wird diese jedoch durch Transkribus-Dokument-ID abgelöst. Beide IDs werden zur besseren Nachvollziehbarkeit in der CSV notiert.
...
\begin{itemize}
  ...
  \item Signatur
\end{itemize}
Die in \nameref{Dokumententypen} dargestellten Kategorisierungen werden nun als Groundtruth-Daten in die CSV aufgenommen, um sie später in der Pipeline auszuwerten. Das bedeutet, sie gelten damit in den weiteren Schritten als verbindliche Referenz zur Validierung und Auswertung.

\subsection{Digitalisierung der Quellen}

Überlieferte analoge Dokumente müssen zunächst fachgerecht für das Projekt und den Digitalisierungsprozess aufbereitet werden. 
Hierzu werden die Akten aus ihren ursprünglichen Ablagesystemen entnommen und sorgfältig von Gummibändern, Büro- und Heftklammern 
...
Es extrahiert die Seiten, speichert sie im geeigneten 
...
Ausschlaggebend ist insbesondere die Möglichkeit, ein eigenes HTR-Modell auf Basis einer projektspezifischen Groundtruth zu trainieren, 
...
\begin{minipage}[t]{0.52\textwidth}
  ...
  \justifying
  \noindent
Wie das Beispiel \autoref{fig:Carl-handschrift} rechts verdeutlichen soll, sind viele der Akten schwer entzifferbar. Auch Transkribus kommt wegen der kleinen
Sets unterschiedlicher Autoren und Autorinnen für spezifische Handschriften an seine Grenzen. Mit Expertise sind diese nur mit hohem zeitlichen Aufwand transkribierbar. 
...
Schriftsetzung einen höheren Aufwand in der Transkription benötigen. Ausgangspunkt für dieses Projekt ist das
\noindent
\end{minipage}%
\hfill%
...
generische Modell \textit{The German Giant I}, 
...
sind dabei jedoch sehr unpräzise, wie \autoref{fig:Carl-Transkribus} veranschaulicht. 
In insgesamt vier Transkriptionsdurchläufen über diese Selektion wird daher manuell eine Groundtruth für ein eigenes Modell erstellt und gleichzeitig regelbasiert getaggt. Die Tags erfassen strukturiert Personen, Orte, Daten und
Organisationen. Ein eigenes Kapitel hierzu findet sich im Anhang \nameref{subsec:tagging_chatpter}. Für die OCR-Erkennung wird eingangs auch ChatGPT verwendet, das in der direkten Texterkennung aus Bilddateien als nicht ausreichend zuverlässig bewertet werden muss.\footnote{Ausführlich wird dies in \nameref{subsec:OpenAI} erläutert.}

\begin{minipage}[t]{0.52\textwidth}
  \setstretch{1.5}
  \justifying
  \noindent
Bei der oben ausgeführten
Erstellung des Groundthruth-Korpus wird bei der manuellen Korrektur OpenAIs ChatGPT-4o-Modell für die Rechtschreibprüfung verwendet. Die vermeintliche Schwäche 
bei der Transkription, passende Begriffe zu halluzinieren, stellt sich hier als besonders hilfreich heraus. Gerade bei der Rekonstruktion fehlender Wörter oder Satzteile zeigt sich die Stärke des wahrscheinlichkeitsbasierten LLMs.
...
bei dem Maschine und Mensch sich wechselseitig ergänzen. Die automatische Transkription wird
\end{minipage}
\vspace{1em}
\begin{minipage}[t]{0.5\textwidth}
  ...
  \begin{tcolorbox}[colback=oldLetter, colframe=black, sharp corners, width=0.8\textwidth]
    ...
    Carl} 
  \end{tcolorbox}
  \captionof{figure}{Transkription durch \textit{The German Giant I}-Modell von Transkribus der  \autoref{fig:Carl-handschrift}\label{fig:Carl-Transkribus}}
\end{minipage}
\hfill%
\vspace{0.76cm}
\noindent in \autoref{fig:Carl-Transkribus} dargestellt, die überarbeitete LLM-Version folgt in \autoref{fig:Carl-LLM}.
...
Transkription unabdingbar,wobei sich die knapp 1.7\% geringere CER sich jedoch beim Korrekturaufwand bereits bemerkbar macht. Gleichzeitig wird diese Korrektur für das Taggen benutzt, das folgend beschrieben werden soll.
...
\end{tcolorbox}
  \captionof{figure}{Durch ChatGPT unterstützte Transkription von \autoref{fig:Carl-Transkribus}.\label{fig:Carl-LLM}}
\end{minipage}
\hfill%
...
\subsubsection{Tagging mit Transkribus und LLM}\label{subsec:tagging_chatpter}
...
die von einer historisch geschulten Person geprüft und übernommen oder verworfen werden. Dieser kollaborative Vorgang verbessert 
...
wird zunächst eine maschinelle Transkription erstellt (\autoref{fig:Carl-Transkribus}), die anschliessend durch ein LLM geglättet und lesbarer gemacht wird (\autoref{fig:Carl-LLM}).
...
\begin{minipage}[t]{0.52\textwidth}
  ...
  \justifying
  \noindent
In einem letzten Schritt erfolgt die manuelle Annotation mit Transkribus-Tags (\ref{fig:Tagging-Carl-LLM}): 
...
zwischen Person, Ort und kulturellem Kontext aufgeschlüsselt. Für unklare oder unleserliche Textstellen, wie etwa das Fragment \texttt{"auf Wiederschen"}[sic], 
kommt das Tag \texttt{unclear} zum Einsatz – häufig auf Grundlage einer Vorschlagsformulierung durch ChatGPT. Zusätzlich zur semantischen Markierung ermöglicht Transkribus auch die Kennzeichnung struktureller Eigenschaften.
\end{minipage}
\begin{minipage}[t]{0.5\textwidth}
    \centering
    \vspace*{0cm}
...
\end{tcolorbox}
  \captionof{figure}{Getaggte Transkription von \autoref{fig:Carl-LLM}.\label{fig:Tagging-Carl-LLM}}
\end{minipage}
...
Die Auswahl der Tools orientiert sich dabei an Kriterien wie Verfügbarkeit (Open~Source vs.\ proprietär), 
...
Um diese Unsicherheiten zu dokumentieren, werden hier auch gescheiterte Versuche dargestellt. Dazu gehört der nachfolgende Linked-Open-Data-Ansatz.
...
Das Potenzial wird diese Form der Datenverknüpfung bislang nicht von allen Websites konsequent 
umgesetzt.\cite[ vgl.][S. 14]{garoufallou_metadata_2020}. Für die technische 
Umsetzung für diese Arbeit werden zwei zentrale Werkzeuge erprobt. Das sind Protégé zur Modellierung der Ontologie und GraphDB für deren Verwaltung und Abfrage.

\subsubsection{Protégé} Zur praktischen Modellierung der Ontologie kommt \textit{Protégé} zum Einsatz, bis sich der Fokus der Arbeit im weiteren Verlauf verlagert. 
...
Die initiale Version der Ontologie für dieses Projekt entsteht im Codeeditor \textit{Visual Studio Code,} wird aber schnell vollständig in Protégé überarbeitet.
Damit bildet das Programm die Grundlage für erste Experimente mit Abfragen in SPARQL.%

\subsubsection{GraphDB} Für die Speicherung und Abfrage der Ontologie wird \textit{GraphDB} verwendet. 
...
Im Rahmen dieser Arbeit dient GraphDB als Backend, um die in Protégé entwickelte Ontologie zu testen 
...
Aus diesem Grund wird auf der Basis der oben beschriebenen Semantik begonnen, eine eigene Ontologie zu entwickeln, die die identifizierten Entitäten systematisch erfasst.
...
und der \enquote{Suchdienst des DRK}\cite[ vgl.][]{reuter_drk_2025}. In beiden Fällen liegen die Daten jedoch nicht als LOD vor. Das Forum stellt einfache Strings dar, das Deutschen Roten Kreuz OCR-PDF\footnote{OCR = Optical Character Recognition} historischer Suchlisten aus der Nachkriegszeit. Ein manuelles Recherchieren dieser Daten scheint zu diesem Zeitpunkt
...
Wikidata\cite[vgl.][]{noauthor_wikidata_nodate} ist eines der zentralen Repositorien für Linked Open Data und bietet eine hohe Interoperabilität durch standardisierte
...
Die praktische Umsetzung offenbart jedoch eine strukturelle Einschränkung: Etwa 70\% der für diese Arbeit eigens angelegten Einträge auf Wikidata werden trotz systematischer Verknüpfung mit anderen dort verwalteten Entitäten\footnote{etwa Armeen, Militäreinheiten, Orten und Personen} im Rahmen der Community-Moderation wieder entfernt.
Das zeigt einerseits die hohen internen Qualitätsanforderungen auf, andererseits werden diese jedoch nicht klar kommuniziert. Mit rigidem Löschen neuer Einträge wird die Verlässlichkeit und der Nutzen der 
geleisteten Arbeit erheblich begrenzt. 
Aufwand und Unsicherheit über die Persistenz der Einträge machen den ursprünglich vorgesehenen LOD–Ansatz in dieser Form nicht praktikabel. Daher findet es innerhalb der Pipeline optionale Anwendung. Wenn einzelne Entitäten eine Wikidata-ID haben, wird diese in die \nameref{subsec:Nodegoat_chapter}-Datenbank aufgenommen und für das Entity Matching verwendet.
...
\nameref{subsec:place_matcher_chapter} beschrieben ist. So werden mögliche Ortsnamen als Kandidaten beispielsweise automatisch in diesem Modul per GeoNames-API abgefragt.  Im Gegensatz zu Wikidata wird hier von Beginn an darauf verzichtet, eigene Ortsdatensätze zu ergänzen. Dies liegt einerseits an den klar kommunizierten Community Guidelines und andererseits daran, dass der Datensatz, mit Ausnahme einiger sehr lokaler Flurnamen\footnote{Der \enquote{Totenbühl} in Murg ist beispielsweise ein solcher Flurname}, als weitgehend vollständig gelten kann. Da GeoNames auf aktuelle Daten ausgerichtet ist, fehlen historische Gebäude wie Gaststätten oder Spitäler konsequenterweise in der Datenbank. Diese Lücke ist zwar erwartbar, bleibt jedoch im Rahmen historischer Auswertungen relevant.
...
hier besonders:\\][]{gubler_Nodegoat_nodate} oder, wie unten geschildert, auf konkrete Nachfrage bei den Entwicklern. Der so geleistete Support 
ist jedoch ausgesprochen zeitnah und umfassend. Beispielsweise wird der IFrame des Public Interface wegen dieses Projekts so angepasst, dass man die Anzeigetools nun optional deaktivieren kann. Eine Verbesserung von Nodegoat, von der auch weitere User in Zukunft profitieren können.\footnote{implementiert wird diese Funktion im September 2025.}
...
Für das hier behandelte Forschungsvorhaben übernimmt Nodegoat die zentrale Verwaltung der Groundtruth-Daten, die später als CSV-Export in die Pipeline integriert werden. 
...
noch vielfältigere Abbildung abhängiger Informationen. Das können in dieser Arbeit zum Beispiel Quellenbelege sein, sowie temporale oder lokale Attribute, die einem Hauptobjekt zugeordnet werden. Lokale Attribute 
werden in Verknüpfung mit \hyperref[subsec:geonames]{GeoNames} und für Länder, Flurnamen oder Gewässer im Geojson-Format erstellt.\cite[Weiterführend:][]{thomson_geographic_2017} So lassen sich beispielsweise für\code{Persons}
in den Sub-Objekten \code{Geburt} oder \code{Tod} das Datum und der Ort modellieren, sowie die Todesursache notieren.
...
Die strukturierte Erfassung dient dabei nicht nur der internen Qualitätssicherung, sondern ermöglicht auch eine nachhaltige Referenzierbarkeit. Erreicht wird das durch eindeutig 
...
Die Software basiert seit Version 4 auf einem LSTM-basierten\cite[Abk.: \textbf{LSTM} steht für \textit{Long Short-Term Memory};  Architektur der frühen Generation rekurrenter neuronaler Netzwerke \textit{RNNs}. LSTMs wurden entwickelt, um Sequenzdaten zu verarbeiten und dabei sowohl kurzfristige als auch langfristige Abhängigkeiten in der Datenfolge zu erfassen – ein typisches Beispiel sind Texte, Sprache, oder Handschrift; vgl.][p.1-2]{beck_review_2020} neuronalen Netzwerk, das besonders bei der Erkennung von zusammenhängenden gedruckten Textzeilen eine 
...
Passagen auf Postkarten und Zeitungsartikeln genannt werden, die zu massiven Erkennungsfehlern führen. Auch mit angepassten Segmentierungsparametern kann keine zufriedenstellende Texterkennung erzielt werden.

Tesseract wird daher nicht weiterverwendet.

\subsubsection{LLM}\label{subsubsec:LLM_transcript}
Analog zum Einsatz von Python\footnote{siehe Abschnitt \nameref{subsubsection:Tesseract}} stellt die Integration von Large Language Models (LLMs) von Beginn an einen zentralen Bestandteil der Projektkonzeption dar.
...
erhebliche Probleme bei der semantischen Genauigkeit. Das LLM beginnt schnell mit sinnverändernden Haluzinationen, die unklare Textpassagen aus dem gelernten Kontext stimmig auffüllt.
...
Zum Zeitpunkt der explorativen Nutzung stellt der integrierte Content-Filter der Modelle das zentrale Hindernis dar. Inhalte mit Bezug zum Nationalsozialismus
...
Transkribus ist eine webbasierte Plattform zur automatisierten Handschriftenerkennung (HTR) und Texterkennung (OCR), die sich seit ihrer Entwicklung im EU-Projekt READ 
...
Insbesondere bei Listenobjekten, etwa für Personenverzeichnisse oder Inventare, bleibt die XML-Struktur häufig leer. Dies lässt sich zwar durch den Export per Transkribus-API umgehen, die löst jedoch neue Probleme aus. Nach ersten Erkenntnissen scheint das so produzierte PageXML nicht den gängigen Konventionen zu entsprechen, wodurch im Preprocessing durch das LLM massive Störungen für Listen produziert werden.\footnote{siehe \nameref{subsec:OpenAI}, Eine Liste aller fehlerhaften Dateien ist im Anhang \ref{subsec:fehlerhafte_xml}} Sie werden daher ohne Preprocessing durch das LLM in unbearbeiteten der Version des Transkribus Exports verwendet.

Diese Einschränkungen werden auch in aktuellen Studien festgestellt. So verweisen Capurro et al.\cite[][]{capurro_experimenting_2023} im Rahmen ihrer Analyse mehrsprachiger Handschriftenkorpora 
...
Dementsprechend wird OpenAIs LLM in diversen konzeptionellen und operativen Phasen dieses Projektes verwendet. Konkrete Beispiele hierfür finden sich unter anderem in dem Kapitel \nameref{subsec:preprocessing}.

\subsection{SCHREIBEN \colorbox{red}{Webtool}}\label{subsec:webtool_chapter}
...
Model-Endpunkt kombiniert (z.B. \code{https://api.Nodegoat.dasch.swiss/model/type/11680}), um  Labelstrukturen oder benutzerdefinierte Felder korrekt abzubilden.
...
Als zentrale Übersicht gliedert sich diese Darstellung in drei farblich differenzierte Ebenen.
...
verarbeitet und dem Modell GPT-4o übergibt. Grundlage ist ein präzise formulierter Prompt \footnote{vgl. Anhang \nameref{subsec:LLM-Promt}}, der die Struktur des zu erwartenden
...
sowie autoren- und empfängerbezogene Markierungen (\code{author}, \code{recipient}, \code{creation\_place}). Für jedes Tag gibt es genaue Anweisungen
...
Die Antwort des Sprachmodells ist eine vollständige XML-Datei, die sämtliche bestehenden Strukturinformationen beibehalten soll. 
...
Das Skript \code{Transkribus\_to\_base.py} bildet das zentrale Haupt- oder Orchestrierungsmodul des Projekts. Es ist der Ausgangspunkt für alle nachgelagerten Verarbeitungsschritte\footnote{vgl. \nameref{subsec:Module_im_Detail}} und ermöglicht erst deren korrekte Ausführung, indem es systematisch auf die spezialisierten Extraktions- und Validierungsmodule zugreift. Es handelt sich damit um das entscheidende Modul der gesamten Pipeline, dessen Funktionsweise im Folgenden beschrieben wird.
Das Modul überführt die aus Transkribus exportierten XML-Dateien in eine validierte JSON-Repräsentation, die dem projektspezifischen Basisschema (\nameref{subsec:document_schema}) entspricht. Die JSON-Daten bilden die Grundlage für weitere Analysen und der Darstellung auf der Projektwebsite, wobei alle erkannten Entitäten eindeutig und konsistent modelliert werden.

\paragraph{Initialisierung und Konfigurationsprüfung}
Bevor die eigentliche Verarbeitung eines XML-Dokuments beginnt, lädt das Modul Informationen zu \code{Personen}, \code{Orten}, \code{Organisationen} und \code{Rollen}, die aus \textit{Nodegoat}-Exporten stammen. Diese Dateien werden jeweils am Anfang des Skripts eingelesen und für die gesamte Dauer des Verarbeitungsprozesses im Speicher gehalten.
...
Das Modul ist dabei nicht nur auf die Verarbeitung einzelner XML-Dateien beschränkt, sondern analysiert automatisiert ganze Aktenverzeichnisse. Über die Funktion \code{process\_transkribus\_directory()} werden alle enthaltenen Unterordner rekursiv durchsucht, wobei die enthaltenen XML-Dateien jeweils identifiziert, eingelesen und verarbeitet werden. Die Pipeline sucht gezielt nach Daten, die die Vorverarbeitung durch \code{llm\_xml\_preprocessing.py} durchlaufen haben. Die Steuerung erfolgt dabei über die in der Vorverarbeitung entstandenen Ordnerstruktur. Diese wird nach der siebenstelligen Akten-ID und dem Unterordner \code{pages} durchsucht. Anschliessend werden die dort hinterlegten die XML-Dateien geöffnet und verarbeitet. Die Funktion \code{process\_subdirectories()} ermöglicht dabei die getrennte Verarbeitung einzelner Seiten einer Akte, während \code{main()} als zentraler Einstiegspunkt fungiert und den gesamten Batch-Verarbeitungsprozess steuert.

\paragraph{XML-Verarbeitung und Dokument-Identifikation}
Die Verarbeitung beginnt daher mit dem Einlesen der XML-Datei. In einem ersten Schritt werden aus dem XML-Header 
Informationen wie \code{docId}, \code{pageId}, die Transkribus-ID (\code{tsid})sowie Referenzen zu Bild- und Quelldateien extrahiert. Diese Metadaten 
bilden die Basis für die eindeutige Identifikation jeder Seite. Zusätzlich wird aus dem Dateinamen mit einem Lookup in der \code{Akten\_Gesammtübersich.csv} das Dokumentformat (z.B. Brief, 
Postkarte, Protokoll) abgeleitet. Dieses wird später im Feld \code{document\_type} gespeichert und dient der Klassifikation und zielgerichteter Weiterverarbeitung.\footnote{beispielsweise im \nameref{subsection:assigned_roles_module}}

\paragraph{Extraktion des Transkripts}

Parallel dazu wird der vollständige Transkriptionstext aus den \code{<TextEquiv>} bzw. \code{<Unicode>}-Blöcken extrahiert. Der daraus resultierende Fliesstext bildet die zentrale Grundlage für alle nachgelagerten Analyseverfahren. 

Die in den einzelnen \code{TextLine}-Elementen gespeicherten \code{custom}-Attribut   werden im nächsten Schritt durch die zentrale Funktion \code{extract\_custom\_attributes()} systematisch ausgelesen. Jede Zeile wird dabei einzeln auf enthaltene Custom-Tags geprüft. Wird ein solches erkannt, erfolgt eine strukturierte Extraktion der in den Attributen angegebenen \code{offset}- und \code{length}-basierten Textsegmente.
\paragraph{Verwendung interner Hilfsfunktionen}
...
Beispielsweise extrahiert die Funktion \code{extract\_wikidata\_id()} maschinenlesbare Wikidata-Referenzen aus den \code{custom}-Attributen, während \code{parse\_custom\_attributes()} die Attributstruktur einzelner \code{TextLine}-Elemente in ein standardisiertes Dictionary überführt. Weitere Funktionen wie \code{normalize\_role\_name()}, \code{correct\_swapped\_name()} oder \code{get\_place\_name()} sorgen für eine einheitliche Formatierung von Rollenbezeichnungen, Personennamen und Ortsangaben.
...
\paragraph{Standardstruktur und Groundtruth}
Alle extrahierten Entitäten werden in einer standardisierten Datenstruktur gesammelt, die nach fünf Kategorien gegliedert ist: \code{persons}, \code{roles}, \code{organizations}, \code{places} und \code{dates}. Diese bilden das zentrale Ausgangsmaterial für alle nachfolgenden Verarbeitungs- und Validierungsschritte sowie für die finale Konvertierung in ein \code{BaseDocument}-konformes JSON-Format. Die Extraktion selbst wird durch spezialisierte Subfunktionen durchgeführt, die jeweils auf eine bestimmte Entitätskategorie ausgerichtet sind.

Dabei greifen die Extraktionsfunktionen intern jeweils auf die inital geladenenen Groundtruth-Dateien zurück.
Zusätzlich zu Custom-Tags werden Organisationen direkt im Fliesstext mittels \texttt{match\_organization\_from\_text()} identifiziert und in die Datenstruktur übernommen. Grund hierfür ist das fehlende Training des LLMs für nationalsozialistische Organisationen und deren Akronyme. 

\paragraph{Fallback mit Fliesstextextraktion}
...
Besonders betrifft dies Organisationen, die mittels \code{match\_organization\_from\_text()} identifiziert und in die Datenstruktur übernommen werden. Grund hierfür ist das fehlende Training des LLMs für nationalsozialistische Organisationen und deren Akronyme. Auch Rollenbezeichnungen, die in unmittelbarer Nähe zu Personennamen vorkommen, werden  im Kontext so regelbasiert durch \code{assign\_roles\_to\_known\_persons()} extrahiert. Auch hier wird das Ergebnis validiert 
...
\paragraph{Debugging und Logging nicht auflösbarer Entitäten}

Während der Verarbeitung werden potenziell fehlerhafte oder unvollständige Entitäten automatisch markiert und protokolliert. Dazu zählen insbesondere Personen ohne Namen, mit unklarer Rollenbezeichnung oder nicht eindeutiger Zuordnung. Solche Fälle erhalten in den jeweiligen Modulen einen \code{match\_score} unterhalb definierter Schwellenwerte sowie ein entsprechendes \code{review\_reason}, um sie für eine manuelle Nachbearbeitung zu kennzeichnen.

Die Funktion \code{log\_unmatched\_entities()} sammelt alle nicht gematchten oder unvollständigen Entitäten und speichert sie in einer separaten Datei \code{unmatched.json}. Dadurch wird eine systematische Qualitätssicherung und Nachkorrektur der Extraktionsergebnisse ermöglicht. Zusätzlich erfolgen gezielte \code{[DEBUG]}-Ausgaben in der Konsole, die auf kritische Fälle hinweisen und die manuelle Überprüfung unterstützen.

Dieser Logging-Mechanismus bildet die Brücke zwischen automatisierter Extraktion und kuratierender Nachbearbeitung und ist damit ein zentrales Qualitätsinstrument der Pipeline. Auf dieser Basis kann, wie im Kapitel \ref{subsec:unmatched_logger} beschrieben, die Groundtruth erweitert werden.
...
Auf diese Weise kann das Hauptprogramm \code{transkribus\_to\_base.py} auf alle notwendigen Komponenten mit einem einzigen Importbefehl zugreifen, ohne die internen Modulpfade kennen zu müssen. Die Datei gewährleistet somit eine  Trennung zwischen interner Modulstruktur und externer Nutzung, was zur Übersichtlichkeit beiträgt. 

\subsubsection{document\_schemas.py}\label{subsec:document_schema}
Neben der technischen Konsistenz bei der Zusammenarbeit der Module braucht es auch eine inhaltliche Qualitätssicherung aller extrahierten Inhalte aus den Transkribus-Dokumenten des Projekts. Hierfür definiert das Modul \code{document\_schemas.py} die zentrale Schema- und Datenstruktur 
zur Modellierung. Es gewährleistet die einheitliche Repräsentation, 
...
darunter z.B. Gesangsvereine, und NS-Organisationen. Neben dem Namen, Typ und eventuellen 
Alternativnamen (\code{alternate\_names}) wird insbesondere die eindeutige \code{Nodegoat\_id} gespeichert. Für den Spezialfall 
militärischer Einheiten enthält die Klasse ein separates Feld \code{feldpostnummer}. Da sie in der Pipeline jedoch nicht separat getaggt und verarbeitet sind,
...
Algorithmen daher ein Wort im Nominativ Maskulinum genau so erkennen wie im Akkusativ Femininum. Initialen, Signaturen, Nennung einzelner Vornamen oder Nachnamen und die Verwendung von Spitznamen oder Rollen sind lediglich einige Möglichkeiten, eine Person zu benennen. Daraus ergibt sich für den \code{Person\_Matcher} allein auf heuristischer Ebene eine grosse Komplexität, die im Folgenden nur andeutungsweise dargelegt werden kann. 
Im Laufe dieser Arbeit sind vier Parameter entwickelt worden, welche diese Zuordnung ermöglichen sollen:
\textbf{Vorname,
Nachname,
Rolle,
Geschlecht} \\
...
\begin{minipage}[t]{0.48\textwidth}
  ...
  \begin{enumerate}
    ...
    \small{
    \item Matching
    \item Extraktion von Personendaten
    \item Segmentierung \& Anreicherung
    \item Deduplikation 
...
\paragraph{Blacklist \& Konfiguration}\label{paragraph:Pers_match/Blacklist}
...
Diverse Blacklists statuieren Worte, die etwa nicht menschlich sind (bsp. \texttt{"freundliche Grüsse")}. Zu den Blacklists zählt auch \code{Unmatchable\_Single\_Names}, die auf nicht eindeutig verknüpfbare Namen hinweist.\footnote{\texttt{"Otto"} oder \texttt{"Döbele}" sind so häufig vorkommende Namen, dass sie alleine nicht für ein \nameref{paragraph:Pers_match/Matching} ausreichen: Otto häuft sich mindestens 11 Mal.} \code{Pronoun} und nach Geschlecht zusammengefasste \code{Title\_Tokens}\footnote{\code{TITLE\_TOKENS = MALE\_TITLE\_TOKENS | FEMALE\_TITLE\_TOKENS | NEUTRAL\_TITLE\_TOKENS}} markieren Tokens zur gezielten Weiterverarbeitung in nachgelagerten Matching- und Anreicherungsschritten. Alle Ausschlusslisten werden in \code{NON\_PERSON\_TOKENS} zusammengeführt und dienen der Negativselektion im gesamten Matchingprozess. 

\paragraph{Thresholds, CSV-Laden und Groundtruth-Erstellung}\label{paragraph:Pers_match/Threstholds}

Für das Matching werden zwei essentielle Thresholds definiert, die erreicht werden müssen. \code{forename} muss eine 80\%  Ähnlichkeit, \code{familyname} eine 85\% Ähnlichkeit erreichen. Diese Zahlen ergeben sich durch unterschiedliche Tests. Sie bieten einen Kompromiss aus Toleranz gegenüber beispielsweise OCR-Fehlern heraus, und liefern dennoch präzise Trefferquoten. Alle Personendaten werden als Liste von Dictionaries \footnote{\code{List[Dict[str, str]]}} aus der Groundtruth geladen. Sie werden in die Variablen \code{KNOWN\_PERSONS}, \code{GROUNDTRUTH\_SURNAMES} und \code{GROUNDTRUTH\_FORENAMES} übergeben, um später gegen die genannten Thresholds zu testen.

\paragraph{Namensnormalisierung und Titelerkennung}\label{paragraph:Pers_match/Normalisierung}

Für eine konsistente Weiterverarbeitung werden alle erkannten Namensstrings standardisiert, und auf mögliche Rollenstrings überprüft, weil Rollen oft durch das LLM als Person markiert werden. Daher durchlaufen die Namensstrings zwei Normalisierungen (normalize\_name\_string und normalize\_name). Erstere bereinigt den Text grob und trennt Titel vom Rest. Da in den Quellen oft Initialen vorkommen\footnote{Bsp.:\texttt{"A. Zimmermann"}}, wird zwischen den beiden Funktionen ebenfalls auf eine Initiale geprüft.
...
\paragraph{Extraktion von Personendaten}\label{paragraph:Pers_match/Dataextract}
...
Der übergebene Namensstring wird zu Beginn über eine dynamische RegEx mit \code{Title\_tokens} nach Titeln gesucht. Wird ein solcher Titel erkannt, wird er im Feld \code{title} gespeichert und aus dem Namensstring entfernt. Der verbleibende Name wird auf mögliche Rolleninhalte analysiert, die über \code{extract\_role\_from\_raw\_name()} aus dem \nameref{subsection:assigned_roles_module} ermittelt werden. Hierbei wird exemplarisch der String „Schriftführer Ganter“ in die Bestandteile \code{role=Schriftführer} und \code{name=Ganter} aufgetrennt.
...
Zur systematischen Dokumentation unvollständiger Einträge wird zusätzlich die Funktion \code{get\_review\_reason\_for\_person()} bereitgestellt. Sie prüft, ob für die betreffende Person kritische Felder wie Vorname, Nachname oder Rolle fehlen und erzeugt eine entsprechende Review-Kennzeichnung. Diese Information wird im Feld \code{review\_reason} gespeichert. 

\code{merge\_title\_tokens()} sorgt dafür, dass Titel wie \code{„Frau“} oder \code{„Herr“} korrekt mit dem darauffolgenden Namen verbunden und das Geschlecht basierend auf dem Titel automatisch ergänzt wird. Dies ermöglicht insbesondere in der Vorverarbeitungsphase eine präzisere Initialisierung der Felder \code{title} und \code{gender}, bevor die Einträge an die nachgelagerte Matching-Logik übergeben werden.

Beide Funktionen werden in zur  Nachprüfung im \code{split\_and\_enrich} Block verwendet


\paragraph{Segmentierung und Anreicherung}\label{paragraph:Pers_match/SplitEnrich}

Auf die Extraktion einzelner Personenattribute folgt die zentrale Funktion \code{split\_and\_enrich\_persons()}, die den Übergang von unstrukturierten oder teilstrukturierten Personentokens zu formalisierten Personenobjekten koordiniert. Sie übernimmt Listen roher Einträge – typischerweise extrahiert aus Fliesstexten oder XML-Tags – und durchläuft mehrere Verarbeitungsschritte: Strukturprüfung, Normalisierung, Kontextergänzung und semantisches Matching.

\code{split\_and\_enrich\_persons()} übernimmt die übergeordnete Steuerung von \code{match\_person()}, die für den eigentlichen Abgleich einzelner Personen mit der Groundtruth-Datenbank verantwortlich ist. Erstere bereitet die Einträge so auf, dass diese überhaupt matchfähig werden. Das geschieht etwa durch Zusammenführung getrennter Namensbestandteile, Titelzuordnung, Rollenerkennung aus dem Dokumentkontext oder heuristische Strukturvervollständigung.

Sobald ein Eintrag formalisiert ist, ruft die Funktion intern \code{match\_person()} auf und erhält daraus ein Matching-Ergebnis mit \code{match\_score}, \code{confidence}-Wert und ggf. einer \code{Nodegoat\_id}. Dieses Ergebnis wird entweder direkt übernommen oder, bei unklarer Übereinstimmung, durch zusätzliche Heuristiken wie Namensinversion oder Rollenprüfung weiterverarbeitet. Auf diese Weise verbindet \code{split\_and\_enrich\_persons()} die initiale Rohdatensegmentierung mit der konkreten Identifikation bekannter Personen aus dem Groundtruth.


\subparagraph{1. Vorverarbeitung} 
Zunächst wird jeder Eintrag auf seine Struktur geprüft. Bereits vollständige Objekte mit \code{forename}, \code{familyname} und \code{Nodegoat\_id} werden übernommen und minimal bereinigt. Teilstrukturierte Einträge hingegen werden auf mehrzeilige Namen überprüft, indem benachbarte Tokens wie \code{Herr} und \code{Alfons Zimmermann} zusammengeführt werden. Zusätzlich wird die Funktion \code{merge\_title\_tokens()} aufgerufen, um Titel mit nachfolgenden Namen zu verbinden. In dieser Kombination kann das Geschlecht identifiziert und zu der Person ergänzt werden.

\subparagraph{2. Tokenisierung und heuristische Vervollständigung}\label{subparagraph:Pers_match/Tokenisierung}

Für alle anderen Einträge erfolgt wieder eine Zerlegung: Der Inhalt des \code{name}-Feldes wird in einzelne Tokens aufgespalten, und das erste Token wird auf das Vorkommen typischer Titel wie \code{Frau} oder \code{Herr} geprüft. Wird ein solcher erkannt, wird er als \code{title} gespeichert und das zugehörige Feld \code{gender} entsprechend gesetzt.

Besonders berücksichtigt wird dabei der Fall, dass nach dem weiblichen Titel ein männlicher Vorname folgt – wie etwa in \texttt{„Frau Alfons Zimmermann“}. In solchen Fällen wird der männliche Vorname als nicht zur Person gehörig gewertet und entfernt.\footnote{Dies ist notwendig, weil Frauen in den Quellen häufig relational über ihre Ehemänner adressiert werden: \texttt{Frau Alfons Zimmermann} bezeichnet tatsächlich die \texttt{Frau} \underline{von} \texttt{Alfons Zimmermann}. Ein Beispiel für Genderbias-Verzerrung in historischen Dokumenten, vgl. \cite{burkhardt_bias_2025}.} Anschliessend wird das verbleibende Token als Nachname interpretiert und im Feld \code{familyname} gespeichert; das Feld \code{forename} bleibt in der Regel leer, um fehlerhafte Zuordnungen zu vermeiden.

Werden hingegen keine Titel erkannt, aber der Namensstring ist trotzdem auswertbar, wird der gesamte Inhalt als Nachname behandelt.
Darüber hinaus werden Tokens, die vollständig in einer Blacklist vorkommen durch Vergleich mit \code{NON\_PERSON\_TOKENS} identifiziert und verworfen. interpretiert werden.

Alle so erzeugten oder überarbeiteten Einträge werden in einer neuen Liste \code{processed\_raw\_persons} gesammelt und am Ende des Abschnitts in die Variable \code{raw\_persons} überführt, die dann als Grundlage für die nächste Verarbeitungsstufe dient.

\subparagraph{3. Kontextbasierte Rollenerkennung} 
Mithilfe des (\code{transcript}) werden erkannte Namen im Text lokalisiert. Dazu wird der vollständige Text zeilenweise durchsucht und geprüft, ob der kombinierte Namensstring einer Person in einer der Zeilen enthalten ist. Bei einem Treffer wird die unmittelbar darauf folgende Zeile mit der Funktion \code{infer\_role\_and\_organisation()} analysiert.

Diese Funktion sucht mit RegEx nach typischen Mustern für Rollentitel und überprüft, ob er zusammen mit einer zugehörigen Organisationseinheit genannt wird. Erkennt das System eine solche Struktur, etwa\texttt{Leiter des V.D.A.}, so werden die Felder\code{role} und \code{associated\_organisation} im Personenobjekt automatisch ergänzt.

Neben expliziten Mustern wie \code{<Rolle> des <Organisation>} erkennt die Funktion auch unstrukturierte Rollennennungen anhand von Schlagwortlisten (\code{ROLE\_PATTERNS}) oder typischer Rollenendungen wie \code{-führer}, \code{-leiter}, \code{-wart}. In Fällen, in denen keine Organisation erkannt wird, bleibt das zugehörige Feld leer, während die Rolle trotzdem gespeichert wird. Durch diese Kontextanalyse können auch Rollenbezüge erfasst werden, die im eigentlichen Namenseintrag nicht enthalten waren.

\subparagraph{4. Matching gegen Groundtruth}\label{subparagraph:Pers_match/Matching}

Im nächsten Schritt wird versucht, jede zuvor bereinigte und kontextualisierte Person mit einem Eintrag aus dem Groundtruth abzugleichen. Dafür wird ein \code{raw\_token} gebildet, bestehend entweder aus dem ursprünglich erkannten String oder der Kombination aus \code{forename} und \code{familyname}. Sofern bereits alle Kernfelder (\code{forename}, \code{familyname}, \code{Nodegoat\_id}) vorhanden sind, wird der Eintrag ohne erneutes Matching übernommen und in die Liste \code{matched} eingefügt. Die Daten können bereits aus vorangegangener manueller Annotation in Transkribus oder exakter Extraktion stammen.

Für alle übrigen Personen wird die Funktion \code{extract\_person\_data()}\footnote{siehe Abschnitt \ref{paragraph:Pers_match/Dataextract}} aufgerufen, um aus dem Namen sowie gegebenenfalls bekannten Titeln oder Geschlechtsangaben ein standardisiertes Personenobjekt zu generieren. Dieses Objekt wird an \code{match\_person()} übergeben, wo der eigentliche Abgleich gegen den Groundtruth erfolgt. Wird ein Treffer mit ausreichender Ähnlichkeit gefunden, wird die Person samt \code{match\_score}, \code{confidence}-Label und Nodegoat-ID als gematcht markiert.

Um Redundanzen zu vermeiden, wird zusätzlich ein eindeutiger Schlüssel aus Vor- und Nachnamen oder der ID generiert. Kommt derselbe Schlüssel mehrfach vor, wird der erste gültige Treffer priorisiert. Kann keine eindeutige Übereinstimmung festgestellt werden, wird der Eintrag zur manuellen Nachprüfung in die Liste \code{unmatched} aufgenommen und mit dem \code{review\_reason} versehen.

Als zusätzlicher Fallback wird versucht, durch Tausch von Vor- und Nachname einen potenziellen OCR-bedingten Inversionsfehler zu korrigieren. Wenn auch dieser Abgleich scheitert, wird die Person als ungeklärt markiert. Die Einträge dieser Kategorie können in einem späteren Durchlauf nachbearbeitet oder manuell aufgelöst werden.


\paragraph{Deduplikation }\label{paragraph:Pers\_match/Dedub}

Die Funktion \code{deduplicate\_persons()} konsolidiert alle im Dokument identifizierten Personeneinträge. Ziel ist es, Dopplungen zu erkennen, redundante Einträge zu verwerfen und für jede erkannte Person einen einzigen, mit allen zur Verfügung stehenden Informationen angereicherten Datensatz zu erzeugen. Die Gruppierung basiert dabei entweder auf der eindeutigen \code{Nodegoat\_id}, auf einem vollständig erkannten Namen oder, als letztes Mittel, auf einem generierten Dummy-Schlüssel bei Rolleneinträgen ohne Namensangabe.


\subparagraph{1) Gruppierung:}
Im ersten Schritt wird jede Person einem Gruppenschlüssel (\code{key}) zugeordnet:
\begin{itemize}
\item Wenn vorhanden, wird die \code{Nodegoat\_id} verwendet.
\item Falls Name vorhanden: Kombination aus \code{forename} und \code{familyname}.
\item Falls nur ein Teil des Namens existiert, wird dieser genutzt.
\item Falls keine Namensinformation vorliegt (z.B. nur Rolle), wird ein temporärer Schlüssel der Form \code{role\_only: } erzeugt.
\end{itemize}
Diese Gruppierung wird in einem Dictionary \code{person\_groups} gesammelt.


\subparagraph{2) Auswahl der besten Repräsentation pro Gruppe}
Für jede Gruppe wird ein repräsentativer Haupteintrag gewählt. Die Auswahl erfolgt anhand folgender Hierarchie:
\begin{itemize}
\item Einträge mit gültiger \code{Nodegoat\_id} und hohem \code{match\_score} werden bevorzugt.
\item Andernfalls wird der am besten bewertete Eintrag über Fuzzy-Matching bestimmt.
\item Falls kein eindeutiger Treffer ermittelt werden kann, wird der vollständigste Originaleintrag übernommen und als \code{needs\_review = True} markiert.
\end{itemize}

\subparagraph{3) Anreicherung der Felder:}
Innerhalb jeder Gruppe wird der gewählte Haupteintrag mit zusätzlichen Informationen aus den übrigen Gruppeneinträgen angereichert:
\begin{itemize}
\item \code{mentioned\_count} wird als Summe aller Gruppenvorkommen berechnet.
\item Der höchste Wert für \code{recipient\_score} wird übernommen, um die Wahrscheinlichkeit einer Empfängerschaft zu erfassen.\footnote{Je höher der Score, desto wahrscheinlicher ist die Person der tatsächliche Empfänger des Dokuments.}
\item Alle erkannten \code{role}-Felder werden vereinheitlicht und als Liste zusammengeführt.
\item Leere oder unvollständige Felder wie \code{title}, \code{alternate\_name}, \code{associated\_place} oder \code{associated\_organisation} werden, falls möglich, aus anderen Gruppenmitgliedern ergänzt.
\item Organisationseinträge werden bei Bedarf durch die Hilfsfunktion \code{flatten\_organisation()} aus verschachtelten Strukturen in eine flache Repräsentation überführt.
\item Zusätzlich wird für jede \code{Nodegoat\_id} geprüft, ob aus der Ursprungsliste ein höherer \code{recipient\_score} vorliegt; dieser wird bei Bedarf rückwirkend übernommen.
\end{itemize}


\subparagraph{4) Fallback bei unklaren Matches:}
Falls keine \code{Nodegoat\_id} vorhanden ist, wird ein Fuzzy-Match gegen die bekannte Groundtruth durchgeführt (\code{match\_person()}). Bei Score $\geq 90$ wird der Eintrag angereichert, andernfalls verbleibt er als ungematchter Rohdatensatz. Stimmen entweder Vor- oder Nachname mit einem vorhandenen Eintrag überein, erfolgt zusätzlich eine heuristische Fusion unter Berücksichtigung der Namensähnlichkeit.


\subparagraph{5) Logging und Debugging:}
Für jede nicht gematchte oder manuell überprüfungsbedürftige Person wird ein entsprechender Debug-Eintrag ausgegeben. Besonders fehleranfällige Fälle (z.B. Personen mit Rolle aber ohne Namen) werden mit niedrigem \code{match\_score} und passender \code{review\_reason} markiert. Die Hilfsfunktion \code{get\_best\_match\_info()} dient in diesem Kontext als standardisiertes Protokoll für manuelle Prüfprozesse. Dieser Schritt initiiert die manuelle Nacharbeitung von ungematchten Entitäten, wie sie im \nameref{subsec:unmatched_logger} beschrieben ist.


\subparagraph{6) Kontextuelle Zählung der Nennungen:}
Ergänzend zur initialen Zählung durch die Custom-Tags erfolgt mit \code{count\_mentions\_in\_transcript\_contextual()} eine kontextbasierte Überprüfung im Transkript. Die Suche erfolgt zeilenweise nach vollständigem Namen, Vor- oder Nachname.
Zusätzlich wird geprüft, ob innerhalb von ±1 Zeile die zugehörige Rolle erscheint. Um Doppelzählungen zu vermeiden, wird pro Kontextblock nur eine Zählung vorgenommen.
So wird sichergestellt, dass auch aus dem Fliesstext korrekt abgeleitete Personenbezüge in die \code{mentioned\_count}-Werte einfliessen.
...
\begin{minipage}[t]{0.38\textwidth}
  ...
  \subsubsection{Assigned\_Roles\_Module.py}\label{subsection:assigned_roles_module}
      ...
      Wie im \code{Person\_matcher.py} deutlich wird, ist auch das Erkennen von Rollen essentiell für das Erkennen von Personen. Sei dies um Informationen zu einzelnen Personen anzureichern, oder um sie anhand einer reinen Rollennennung im Text zu identifizieren. Für den vorliegenden Korpus wurden Rollen in fünf Kategorien strukturiert, die in Nodegoat modelliert sind: \textbf{Familie}, \textbf{Politisches Amt}, \textbf{Beruf}, \textbf{Vereins-Rolle}, und \textbf{Militärangehörige}. Durch diese Strukturierung ermöglichen Rollen auch die Definition von Beziehungstypen. Eine Rolle verbindet demnach immer eine Person mit einer Organisation,\footnotemark{} oder etwa durch Verwandtschaft mit einer anderen Person. 

  \end{minipage}%
  \footnotetext{die nicht bekannt sein muss; das ergibt eine Blank-Node}
% ================================
...
Weil Personen und Rollen so eng verknüpft sind, werden im \code{Assigned\_Roles\_Module.py} auch beide Groundtruth-CSV Tabellen initial geladen. Hinzu kommt eine interne Lookup-Tabelle \code{Role\_Mapping\_DE}, welche Synonyme und alternative Schreibweisen von Rollenbezeichnungen enthält. Neben dem Grundformen aus der Groundtruth werden alle Rollen auch in \code{normalize\_and\_match\_role} grammatisch flexiert.\footnote{d.h. dem jeweiligen Kasus angepasst.} Umgesetzt wird dies durch regelbasiertes Stemming mit festgelegten Endungen.\footnote{siehe im Modul: # 3. Maskuline Flexionsendungen zurückführen (z.B. „vorsitzenden“ → „vorsitzender“)
    suffixes = ["en", "ern", "em", "e", "n", "er", "es", "nt", "ner", "ners"]}
Das Verfahren ist gezielt auf das kleine, bekannte Vokabular aus der \code{export-roles.csv} optimiert, insbesondere für die maskuline Rollenform. Weibliche Rollennamen kommen im Untersuchungskorpus ausserhalb der Familienkategorie nur ein einziges Mal vor.\footnote{die Rolle: \texttt{Damenschneiderin}} Der Algorithmus prüft dennoch auch weibliche Endungen, um den Genderbias des Algorithmus zu minimieren. So bleibt der Code für eine potentielle Ausweitung des Korpus zu einem späteren Zeitpunkt robust.

Aus allen erkannten Varianten wird eine vollständige Liste \code{POSSIBLE\_ROLES} erstellt, die als Vokabular für die Rollenextraktion dient. Innerhalb des Korpus treten Rollenbezeichnungen nicht einheitlich auf, sondern variieren. Sie lassen sich in drei Szenarien festlegen, die per Regex komplexe zusammengesetzte Möglichkeiten identifizieren und lösen.

\begin{itemize}
    \item \code{ROLE\_AFTER\_NAME\_RE} \\
    Erkennt Rollen, die einer Person nachgestellt sind. \\
    \textit{Beispiel:} \texttt{Alfons Zimmermann, Vereinsführer Männerchor Murg}
    
    \item \code{ROLE\_BEFORE\_NAME\_RE} \\
    Erkennt Rollen, die einer Person vorangestellt sind, ggf. mit Organisationsbezug. \\
    \textit{Beispiel:} \texttt{Vereinsführer des Männerchors Alfons Zimmermann}
    
    \item \code{STANDALONE\_ROLE\_RE} \\
    Erkennt Rollen, die alleinstehend oder nur mit Organisation genannt werden. \\
    \textit{Beispiele:} \texttt{Vereinsführer} oder \texttt{Vereinsführer Männerchor Murg}
\end{itemize}
...
Nach der Normalisierung wird jede erkannte Rollenbezeichnung mit \code{map\_role\_to\_schema\_entry()} in einen strukturierten Schemaeintrag überführt. Dieser enthält eine eindeutige Rollen-ID aus der Groundtruth, die standardisierte Rollenbezeichnung, die zugehörige Rollenkategorie. Wenn sie aus dem Kontext extrahierbar sind, werden auch Bezüge zu einer Organisation, deren Name oder ID ergänzt. Rollen, die nicht unmittelbar in der Form \emph{Name–Rolle–Organisation} auftreten, werden durch eine gezielte Kontext- und Umfeldauswertung ergänzt. Dazu lokalisiert \code{find\_line\_index\_for\_person()} zunächst die betreffende Person im Transkript, woraufhin \code{search\_roles\_nearby()} definierte Zeilenumfelder auf Rollenhinweise untersucht. Präpositionen wie \textit{des, der, dem, den, im, beim, zum, zur} dienen hierbei sowohl der Bereinigung von Strings als auch als positive Indikatoren für Rollen-Relationen (z.B. „Vereinsführer des Männerchors“). Ergänzend identifiziert \code{extract\_standalone\_roles()} alleinstehende Funktionsangaben und koppelt sie – sofern möglich – über nahe Organisationsnennungen oder globale Heuristiken. Komposita aus Namen und Rollen werden mittels \code{extract\_role\_in\_token()} in ihre Bestandteile zerlegt.
Die so gewonnenen Rollen werden anschliessend mit bekannten Personen aus \code{export-person.csv} und Organisationen aus dem \code{organization\_matcher} verknüpft. Dabei weist \code{assign\_roles\_to\_known\_persons()} Rollen bestehenden Personenobjekten zu und ergänzt fehlende Felder, während Organisationsbezüge extrahiert und mit der Groundtruth abgeglichen werden.

Da Rollen mehrfach oder in unterschiedlichen Varianten auftreten können, erfolgt abschliessend eine Konsolidierung. Hierbei werden identische Rollenbezüge (gleiche Person, gleiche Organisation, gleiche Rolle) zusammengeführt, unsichere Fälle mit den Feldern \code{needs\_review} und \code{review\_reason} markiert und im Zusammenspiel mit dem \code{validation\_module.py} Plausibilitätsprüfungen durchgeführt.
...
Während der \code{Person\_Matcher} hochkomplexe Strukturen aus Vor- und Nachnamen, alternativen Namensformen und zugehörigen Rollen verarbeitet, mag die Identifikation von Ortsangaben auf den ersten Blick vergleichsweise trivial erscheinen. Eine eingehendere Analyse der zugrunde liegenden historischen Quellen zeigt jedoch, dass auch Ortsnamen eine erhebliche Komplexität aufweisen können.

Das beginnt bei der grundlegenden Frage, was als Ort überhaupt zu de-
...
finieren ist. Im Rahmen dieser Arbeit konnten acht Orts-Typen identifiziert werden, die es aus den Dokumenten extrahiert werden müssen:
\code{Kontinente},
\code{Länder},
\code{Regionen},
\code{Siedlungen},
\code{Berge},
\code{Seen},
\code{militärische Stellungen} und
\code{Gebäude}.
Historische Strassennamen und Hausnummern können nicht mehr ohne teils immensen Rechercheaufwand nachvollzogen werden. Da für die Groundtruth der Fokus auf Koordinaten oder GeoJSON Daten liegt, werden sie daher ausgenommen.

Um oben gelistete Entitäten unverwechselbar zu beschreiben, nutzt diese Arbeit eine Kombination mehrerer Identifier. Auf der einen Seite ist das die \code{Nodegoat-ID} aus der Groundtruth. Auf der anderen Seite wird diese durch Verknüpfungen der IDs von \nameref{subsec:geonames} und \nameref{subsec:wikidata} ergänzt. Damit werden die Geo-Daten \textbf{FAIR}\footnote{Findable, Accessible, Interoperable, Retrievable} und sind anschlussfähig an bestehende \footnote{nameref{subsec:LOD}} Infrastrukturen. 

Der \code{Person\_matcher} löst gleich mehrere Probleme bei der Named Entity Recognition und des Entity Matchings. Das grundlegendste Problem ist dabei die Absicherung gegen inkonsistente Schreibweisen. Diese können durch OCR-Fehler entstehen, weil sie historische Schreibweisen oder auch Regionalismen\footnote{Regionalismen: regionale sprachliche Eigentümlichkeiten} sind. Für den Ort \texttt{Murg} existieren beispielsweise 26 unterschiedliche Schreibweisen, die in knapp über ein Dutzend Grundformvarianten\footnote{Darunter etwa \texttt{Murg/Baden}, \texttt{Murg (Baden)}, \texttt{Murg Hochrhein}, \texttt{Murg a.\ HRh.} u.a.} abgeleitet werden können. Die Schreibweise meint hier die Verwendung von Zeichen, die Grundform eine bereinigte Version.\footnote{aus \texttt{Murg (Baden)} oder \texttt{Murg /Baden} wird immer \texttt{murg baden}, im Ggs. zu \texttt{Murg Hochrhein}, oder \texttt{Murg am Rhein}} Diese Formen müssen für die RegEx-Prüfungen bekannt sein, da diese ansonsten scheitern würden. Essentiell hierfür ist eine verlässliche Groundtruth.

Die Matching-Logik selbst kombiniert mehrere Verfahren. Zunächst erfolgt ein exakter Abgleich mit der Groundtruth, gefolgt von einem Fuzzy-Matching bei OCR-Fehlern oder oben ausgeführter alternativer Schreibweisen. Für nicht erkannte Orte greift ein API-basierter Abgleich über GeoNames und Wikidata, dessen Ergebnisse mit Scorewerten versehen und mit Konfidenzstufen markiert werden. Scheitert auch dieser Lookup, werden die Daten durch den \code{unmatched\_logger.py} verarbeitet und für die manuelle Weiterverarbeitung aufbereitet. 

Auch der \code{place\_matcher} fusst seine internen Verarbeitungsschritte auf das Tagging und der Vorarbeit durch das LLM. Manchmal sind diese Tags jedoch wegen des domänenunspezifischen Trainings zu sensitiv. So können Stadtteile beispielsweise als zwei separate Orte annotiert werden.
Daher ist das Zusammenfügen von möglichen Segmenten zu einem bekannten Ort eine der Stärken des Moduls. Als Beispiel hierfür dient der Ortsname \texttt{Laufenburg-Rhina}. Würde die Pipeline die beiden Teilorte \texttt{Laufenburg} und \texttt{Rhina} als separate Orte erkennen und nicht als gemeinsame Menge \texttt{Laufenburg-Rhina}, wäre das Ergebnis verfälscht. Ein Brief bekäme so absurderweise zwei Orte, die gleichzeitig als Absendeort gelten würden. 
Der \code{place\_matcher} überprüft daher mit einem \code{Content-Window} von  ±1 Wort innerhalb einer Zeile sowie über Zeilengrenzen hinweg auf möglichen Namen oder Alternativnamen. Diese Kombinationen werden einzeln mit der Groundtruth abgeglichen. Das Verfahren erlaubt auch das Matching komplexer Ortsnamen.

Neben den offensichtlichen Matching-Prozessen enthält das Modul auch eine Reihe von Funktionen, die im Hintergrund zur Sicherung der Datenqualität beitragen. So validiert \code{is\_valid\_place\_name()} alle eingehenden Ortsreferenzen auf formale Plausibilität und filtert unbrauchbare oder irrelevante Einträge (z.B. Adjektivphrasen oder Kontextfragmente) konsequent aus. Die Funktion \code{\_extract\_name\_only()} trennt relevante Ortsnamen zudem von begleitenden Funktionswörtern wie „in“, „bei“ oder „aus“, um die Vergleichbarkeit mit der Groundtruth zu erhöhen.

Diese unscheinbaren, aber zentralen Verarbeitungsschritte bilden die Grundlage für die im Anschluss durchgeführte Deduplikation. Hierbei werden erkannte Ortsreferenzen über \code{deduplicate\_places()} zu konsolidierten Einträgen zusammengeführt. Kriterien hierfür sind identische IDs, aber auch Ähnlichkeiten in der normalisierten Schreibweise. Orte, die sich in keiner Form eindeutig zuordnen lassen, werden automatisiert im Logfile \code{unmatched\_places.json} vermerkt und für eine spätere manuelle Prüfung vorbereitet.
...
In einem weiteren Schritt werden \code{<recipient>}-Tags aus dem XML durch \code{extract\_recipient\_from\_custom\_tag()} übernommen, sofern vorhanden. Wie bei den Autoren durchlaufen Rezipienten danach die Funktion \code{resolve\_llm\_custom\_authors\_recipients()}. So werden die Rohdaten zusammengeführt, und anschliessend eine Validierung vorgenommen. Insbesondere unvollständige oder generische Einträge wie \texttt{\enquote{Herrn}} oder häufig vorkommende Namen wie \texttt{\enquote{Otto}}\footnote{die Groundtruth kennt 11 verschiedene Personen namens Otto} werden entweder mit dem Vermerk \code{needs\_review = true} versehen oder durch bereits \code{mentioned\_persons} ersetzt. Diese Ersetzung erfolgt abschliessend in \code{letter\_match\_and\_enrich()}.  Unvollständige Einträge von Autor:innen werden durch vollständigere Einträge aus \code{mentioned\_persons} ersetzt, sofern diese zuvor im Text erkannt wurden. Mit \code{extract\_multiple\_recipients\_raw()} können darüber hinaus potenzielle Empfänger:innen aus dem Transkripttext anhand direkter und indirekter Anredeformen aus den jeweiligen Patternlisten extrahiert werden. Im Gegensatz zu \code{extract\_recipients\_raw()}, das sich auf die ersten Zeilen des Dokuments konzentriert, analysiert die Funktion \code{extract\_multiple\_recipients\_raw()} den gesamten Transkripttext. Sie erkennt direkte und indirekte Anredeformen auch im Fliesstext\footnote{etwa im Kontext von Zwischenempfängern oder weitergeleiteten Briefen} und erweitert damit die Empfängerliste über die Briefkopfregion hinaus. Direkte Formulierungen wie \texttt{Lieber Otto} werden mit \code{recipient\_score = 100} als besonders zuverlässig bewertet. Indirekte Anreden wie \texttt{zu Händen des Herrn Alfons Zimmermann} werden mit Score 70 erfasst. Die Ergebnisse enthalten Vorname, Nachname (falls vorhanden), Rolle (leer), Score und Confidence-Label. Durch die unterschiedlichen Ratings im Recipient-Score können so Staffelungen dargestellt werden.
...
Da während der Transkription in dieser Tabelle auch die Dokumententypen festgehalten wurden, kann über einen einfachen Lookup für jede Seite der zugehörige Typ extrahiert werden. Mögliche Kategorien sind unter anderem \enquote{Brief}, \enquote{Postkarte} und \enquote{Protokoll}.\footnote{siehe Kapitel \nameref{Dokumententypen}} 
...
\texttt{custom}-Attribut einzelner \texttt{<TextLine>}-Elemente eingebettet werden. Diese 
Daten gelten innerhalb des Korpus als zuverlässig, da sie während der manuellen Korrekturprozesse
in Transkribus einheitlich normiert und im Format \texttt{dd.mm.yyyy} ergänzt werden.
...
markiert werden.\footnote{Die Funktionsweise dieser Markierungen sowie die heuristische Auflösung solcher
verkürzter Angaben wird im Anhang \nameref{subsec:Taggingregeln_Anhang} erläutert.} Lässt sich aus dem weiteren 
...
\subsubsection{SCHREIBEN enrich\_pipeline.py}
...
%––––––––––––––––––––––––––––––––––––––––––––––––––––––––––––––––––––––––––––––––––––––%
%––––––––––––––––––––––––––––––––––––––––––––––––––––––––––––––––––––––––––––––––––––––% 
\section{SCHREIBEN Analyse \& Diskussion der Ergebnisse}
...
%––––––––––––––––––––––––––––––––––––––––––––––––––––––––––––––––––––––––––––––––––––––%
%––––––––––––––––––––––––––––––––––––––––––––––––––––––––––––––––––––––––––––––––––––––% 
\section{SCHREIBEN Fazit und Ausblick}
...
%Sic   
    ...
    Mit dem Tag \texttt{\colorbox{person}{person}} sind alle Strings getaggt, die eine direkte Zuordnung einer Person ermöglichen.
...
% Event
    ...
    Mit dem Tag \texttt{\colorbox{eventTag}{event}} werden explizite und implizierte Ereignisse markiert. Diese Ereignisse haben einen zeitlichen oder räumlichen Bezug, und können benannt werden. Dazu zählen:
...
\subsection{Übersicht fehlerhaft verarbeiteter Listen in XML}\label{subsec:fehlerhafte_xml}
\tiny{
\begin{table}[ht]
\resizebox{\textwidth}{!}{
\begin{tabular}{|l|l|l|}
\hline
\rowcolor{LightGrey}\textbf{Dokument ID} & \textbf{Dateiname} & \textbf{Fehlerbeschreibung} \\ \hline
\multirow{\textbf{6478535}} & 0001\_p001.xml & \textbf{\textbf{mismatched tag:}} line 279, column 18 \\ 
                         & 0002\_p002.xml & \textbf{mismatched tag:} line 132, column 18 \\
                         & 0003\_p003.xml & \textbf{mismatched tag:} line 186, column 18 \\
                         & 0004\_p004.xml & \textbf{mismatched tag:} line 202, column 18 \\ \hline
\rowcolor{VeryLightGray}\multirow{\textbf{6478650}} & 0059\_p061.xml & \textbf{unclosed token: }line 334, column 16 \\ \rowcolor{VeryLightGray}
                         & 0060\_p062.xml & \textbf{unclosed token: }line 313, column 16 \\ \rowcolor{VeryLightGray}
                         & 0061\_p063.xml & \textbf{unclosed token: }line 334, column 16 \\ \rowcolor{VeryLightGray}
                         & 0062\_p064.xml & \textbf{unclosed token: }line 327, column 16 \\ \hline
\multirow{\textbf{6478888}} & 0002\_p002.xml & \textbf{mismatched tag:} line 240, column 18 \\
                         & 0003\_p003.xml & \textbf{mismatched tag:} line 181, column 18 \\
                         & 0004\_p004.xml & \textbf{unclosed token: }line 334, column 16 \\
                         & 0005\_p005.xml & \textbf{mismatched tag:} line 239, column 18 \\ \hline
\rowcolor{VeryLightGray}\multirow{\textbf{6478907}} & 0001\_p001.xml & \textbf{unclosed token: }line 558, column 20 \\ \hline
\multirow{\textbf{6478947}} & 0002\_p002.xml & \textbf{mismatched tag:} line 288, column 18 \\ \hline
\rowcolor{VeryLightGray}\multirow{\textbf{6478955}} & 0001\_p001.xml & \textbf{mismatched tag:} line 300, column 18 \\ \hline
\multirow{\textbf{6479034}} & 0003\_p004.xml & \textbf{mismatched tag:} line 700, column 18 \\ \hline
\rowcolor{VeryLightGray}\multirow{\textbf{6489797}} & 0002\_p003.xml & \textbf{unclosed token: }line 510, column 20 \\ \rowcolor{VeryLightGray}
                         & 0003\_p005.xml & \textbf{no element found: }line 562, column 86 \\ \hline
\multirow{\textbf{6489822}} & 0001\_p001.xml & \textbf{mismatched tag:} line 220, column 18 \\ \hline
\rowcolor{VeryLightGray} \multirow{\textbf{6489950}} & 0002\_p002.xml & \textbf{duplicate attribute:}line 206, column 72 \\ \rowcolor{VeryLightGray}
                         & 0004\_p004.xml & \textbf{duplicate attribute:}line 381, column 72 \\ \rowcolor{VeryLightGray}
                         & 0008\_p008.xml & \textbf{duplicate attribute:}line 59, column 70 \\ \rowcolor{VeryLightGray}
                         & 0012\_p012.xml & \textbf{duplicate attribute:}line 38, column 70 \\ \hline
\multirow{\textbf{6489950}} & 0014\_p014.xml & \textbf{unclosed token: }line 585, column 16 \\
                         & 0019\_p019.xml & \textbf{duplicate attribute:}line 115, column 72 \\
                         & 0021\_p021.xml & \textbf{duplicate attribute:}line 220, column 72 \\
                         & 0023\_p023.xml & \textbf{duplicate attribute:}line 199, column 72 \\ \hline
\rowcolor{VeryLightGray}\multirow{\textbf{6489950}} & 0025\_p025.xml & \textbf{not well-formed}(invalid token): line 38, column 77 \\ \rowcolor{VeryLightGray}
                         & 0029\_p029.xml & \textbf{duplicate attribute:}line 87, column 72 \\ \rowcolor{VeryLightGray}
                         & 0033\_p033.xml & \textbf{duplicate attribute:} line 150, column 72 \\ \hline
\multirow{\textbf{6490022}} & 0001\_p001.xml & \textbf{unclosed token: }line 370, column 16 \\ \hline
\rowcolor{VeryLightGray}\multirow{\textbf{6490022}} & 0002\_p002.xml & \textbf{unclosed token: }line 313, column 16 \\ \hline
\multirow{\textbf{6490127}} & 0001\_p001.xml & \textbf{unclosed token: }line 376, column 16 \\
                         & 0002\_p002.xml & \textbf{unclosed token: }line 333, column 16 \\ \hline
\rowcolor{VeryLightGray}\multirow{\textbf{6490202}} & 0002\_p002.xml & \textbf{unclosed token: }line 348, column 16 \\ \rowcolor{VeryLightGray}
                         & 0003\_p003.xml & \textbf{unclosed token: }line 361, column 16 \\ \hline
\multirow{\textbf{6490214}} & 0002\_p002.xml & \textbf{unclosed token: }line 320, column 16 \\ \hline
\end{tabular}}

\caption{Übersicht der fehlerhafter Listen-XML.}
\label{tab:fehler_transkribus}
\end{table}
