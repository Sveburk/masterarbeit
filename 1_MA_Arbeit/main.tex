...
\node[source, below= of ROLE] (PERSONGT)  {\textbf{Nodegoat-Export-Persons} \\ (Groundtruth Personen)};
...
\node[source, right=3.5cm of search] (ROLEGTSOURCE)  {\textbf{Nodegoat-Export-Roles} \\ (Groundtruth Rollen)};
...
\end{tikzpicture}
  ...
  \caption{\small\textit{Prozessdiagramm für Assigned\_Roles\_Module.py; schematische Funktionsreihenfolge und Ressourcen}}
  \label{fig:assigned_roles_module}

  \smallskip
  \small\textit{Oben rechts: Pipelineübersicht \\ Rechts: Ressourcen; Role\_Mappings\_DE, Nodegoat-Export-Persons, Nodegoat-Export-Roles}
\end{figure}
