...
\usepackage{nameref}
\usepackage{ifpdf}   % für \ifpdf
\usepackage{iftex}   % für \ifPDFTeX
...
\else
  \usepackage{fontspec} % Für XeTeX & LuaTeX
\fi
...
Die vorliegenden analogen Dokumente müssen zunächst sachgerecht für den Digitalisierungsprozess vorbereitet werden. Zu diesem Zweck werden die Akten aus ihren ursprünglichen Ablagesystemen entnommen und behutsam von Heftklammern und Gummibändern befreit. Diese Maßnahmen dienen in erster Linie dem langfristigen Erhalt des Papiers, da insbesondere Rostspuren ehemaliger Metallklammern die Papierfasern angreifen und irreparable Schäden verursachen können. Darüber hinaus treten häufig Spuren von Säurefraß auf, sofern nicht säurefreies Papier oder unsachgemäßes Archivmaterial verwendet wurde.

Für die eigentliche Digitalisierung wird die native \enquote{Dateien}-App von Apple\footnote{Vgl. \href{https://support.apple.com/de-de/guide/preview/prvw28034/mac}{Apple Support: Dateien-App}} eingesetzt. Diese ermöglicht nicht nur eine vergleichsweise hochauflösende Erfassung, sondern auch die unmittelbare Speicherung in einem Cloud-basierten Speichersystem sowie die automatische Texterkennung (OCR). Ziel ist es, die erfassten Inhalte schnell durchsuchbar und standortunabhängig zugänglich zu machen.

Die Digitalisierung erfolgt mithilfe eines Tablets, das auf einem stabilen Stativ in einem Winkel von 90° über den zu erfassenden Dokumenten positioniert wird. Diese einfache, jedoch effiziente Aufnahmetechnik gewährleistet eine gleichbleibend hohe Bildqualität bei gleichzeitig zügiger Bearbeitung. Die digitalisierten Dateien werden systematisch benannt und orientieren sich an einer zuvor erstellten Gesamtübersicht der Akten. Mehrseitige Konvolute werden als zusammenhängende Akte geführt, während Einzeldokumente entsprechend separat erfasst werden. Die Archivierung erfolgt sowohl in physischer als auch in digitaler Form auf Ebene einzelner Seiten, um eine möglichst feingranulare Erschließung der Bestände sicherzustellen.

Das Dateiformat ist in der \enquote{Dateien}-App standarmässig PDF. Für die anschliessende Transkription
...
und unterschiedlichen LLMs wurde auf Transkribus zurückgegriffen. Eine Gegenüberstellung der drei erwähnten Tools findet sich im Kapitel~\nameref{section:Transkriptionen_Methoden}
...
über militärische Einheiten und deren Feldpostnummern sind das \enquote{Forum der Wehrmacht}\cite[ vgl.][]{altenburger_lexikon_nodate-1} 
