...
image_directory = "/Users/svenburkhardt/Documents/D_Murger_Männer_Chor_Forschung/Masterarbeit/Json_file_test_folder/Test_Data_JSON_files_openai"
output_directory = "/Users/svenburkhardt/Documents/D_Murger_Männer_Chor_Forschung/Masterarbeit/Json_file_test_folder/Test_answer_JSON_files_openai"
...
# Helper function to extract sortable parts from the filename
def parse_filename(filename):
    # Strip the extension and split into parts
    name = filename.rsplit(".jpg", 1)[0]
    # Parse the filename pattern Akte_Zahl_Seite or Akte_Zahl.Zahl_Seite
    parts = name.split("_")
    akte_number = int(parts[1].split(".")[0])  # Main Akte number
    sub_number = int(parts[1].split(".")[1]) if "." in parts[1] else 0  # Optional sub-number
    page_number = int(parts[2][1:])  # Page number (remove "S")
    # Return a tuple for sorting: (akte_number, sub_number, page_number)
    return (akte_number, sub_number, page_number)

# Sort filenames based on parsed numeric values
filenames = sorted(
...
for filename in filenames:
    ...
    It is urgent that you ensure that you output the utf-8 codes as text instead of a character. 
    This is for all German text, especially umlauts (ä, ö, ü) and the ß character, is encoded and outputted 
    in UTF-8 format. Additionally, represent any line breaks in the text as real line breaks rather than `\n`.
    ...
    answer = client.chat.completions.create(messages=workload, model=model, temperature=temperature)
    print("Done.")

    # Extract the answer from the response
    answer_text = answer.choices[0].message.content
    print("> Received an answer from the API. Token cost (in/out):", answer.usage.prompt_tokens, "/", answer.usage.completion_tokens)
    ...
    # The response from the API is a string which encloses the JSON object.
    pattern = r"```\s*json(.*?)\s*```"
