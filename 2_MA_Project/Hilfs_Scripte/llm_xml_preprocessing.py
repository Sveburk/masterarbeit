...
"""
    resp = client.chat.completions.create(
        ...
        messages=[
            {
                "role": "system",
                "content": "You are an XML‐annotation assistant.",
            },
    ...
    if usage is None:
        ...
        cost = round(
            in_tok / 1000 * INPUT_COST_PER_1K
            + out_tok / 1000 * OUTPUT_COST_PER_1K,
            4,
        )
...
def process_file(xml_path: Path, client: openai.OpenAI, failed_log: Path):
    ...
    try:
        ...
        if is_valid_xml:
            out_path = xml_path.with_name(
                f"{xml_path.stem}_preprocessed{xml_path.suffix}"
            )
        else:
            out_path = xml_path.with_name(
                f"{xml_path.stem}_FAILED{xml_path.suffix}"
            )
        ...
        print(
            f"    • Input-Tokens: {meta['input_tokens']}, Output-Tokens: {meta['output_tokens']}"
        )
...
def main():
    ...
    for seven in sorted(TRANSKRIBUS_DIR.iterdir()):
        ...
        for sub in sorted(seven.iterdir()):
            ...
            if all_xmls:
                preproc = [
                    p for p in all_xmls if p.stem.endswith("_preprocessed")
                ]
                ratio = len(preproc) / len(all_xmls)
                if ratio >= 0.5:
                    skipped_files += len(all_xmls)
                    print(
                        f"Überspringe Ordner {page_dir.name}: "
            ...
            to_process = [
                p
                for p in sorted(all_xmls)
                if not p.stem.endswith("_preprocessed")
            ]
            xml_files.extend(to_process)

    print(
        f"Starte LLM-Annotation für {len(xml_files)} Dateien "
